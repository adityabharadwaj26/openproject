...
module Cron
  class ScheduleReminderMailsJob < CronJob
    ...
    def where_statement
      current_timestamp_utc = Time.now.getutc
    ...
    def age_statement(current_timestamp_utc)
      ...
      slot_time = Time.parse(Setting.notification_email_digest_time)
    ...
    def case_statement_zone_name_to_offset
      ...
      statement = ActiveSupport::TimeZone.all.map do |zone|
        offset = current_offset(current_time, zone)
    ...
    # @param [TimeZone] zone
    # @return [String]
    def current_offset(time = Time.current, zone)
